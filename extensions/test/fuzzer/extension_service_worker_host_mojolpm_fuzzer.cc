 ...
 public:
  class LoaderClient : public network::mojom::URLLoaderClient,
   ...
   public:
    explicit LoaderClient(
      ...
      if (body.is_valid())
        body_drainer_ = std::make_unique<mojo::DataPipeDrainer>(
            this, std::move(body));
    }
  ...
  void StartWorker(
    ...
    host_->OnReadyForInspection(
        std::move(agent),
        devtools_agent_host.BindNewPipeAndPassReceiver());
...
void ServiceWorkerHostTestcase::SetUpOnUIThread(
  ...
  activation_token_ = task_queue->GetCurrentActivationToken(
                          extensions::mojolpm::kExtensionIdA)
                          .value();
...
void ServiceWorkerHostTestcase::RegisterContentWorker(
  ...
  const GURL script_url = GURL(
      "chrome-extension://aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/sw.js");
...
void ServiceWorkerHostTestcase::StartContentWorker(
  ...
  context->StartWorkerForScope(
      ...
      base::BindOnce(&ServiceWorkerHostTestcase::FinishContentWorker,
                     base::Unretained(this), std::move(done_closure)),
      base::BindOnce(
          [](content::StatusCodeResponse response) {
...
void ServiceWorkerHostTestcase::RegisterWorker(
  ...
  auto* remote =
      mojolpm::GetContext()
          ->GetInstance<mojo::AssociatedRemote<
              extensions::mojom::ServiceWorkerHost>>(action.id());
  if (remote) {
    event_dispatcher_receiver_.reset();
    auto event_remote =
    ...
    remote->get()->DidInitializeServiceWorkerContext(
        ...
        registered_version_id_, registered_thread_id_,
        worker_token_,
